...
// 音声録音を停止する関数（UI のリセットのみ。wantsToRecordは触らない）
...
// Gemini APIからレスポンスを取得（/api/chat 経由でAPIキーを隠蔽）
async function getGeminiResponse(utterance) {
    ...
    try {
        ...
        const replyText = content.response;
...
async function handleComment(comment, username) {
    ...
    try {
        ...
        // API完了後、録音が途切れていれば再起動（fetch中にstart()が失敗した場合の救済）
